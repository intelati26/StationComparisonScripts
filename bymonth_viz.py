...
def render_calendar(year, month, by_day, sun_times, station_name, tz,
    ...
    # Extra fixed height (inches), added at the BOTTOM only, for the new
    # month-summary footer table -- everything above keeps the exact same
    # absolute size it always had (see the ax_bottom/ax_h derivation below).
    footer_in = 1.55
    fig_h = old_fig_h + footer_in
    ...
    ax_left = 0.015
    ax_w = 0.97
    # The old layout (before the footer existed) was ax_bottom=0.06,
    # ax_h=1.0-0.06-0.10 as fractions of old_fig_h -- i.e. a 0.06*old_fig_h
    # margin, a 0.84*old_fig_h calendar grid, and a 0.10*old_fig_h title
    # band, stacked bottom-to-top. Re-expressed as fractions of the new,
    # taller fig_h (with footer_in inserted between the old margin and the
    # grid) so the grid and title keep their old absolute (inch) sizes and
    # only the footer eats the newly added space.
    old_bottom_margin_in = 0.06 * old_fig_h
    grid_band_in = (1.0 - 0.06 - 0.10) * old_fig_h
    ax_bottom = (old_bottom_margin_in + footer_in) / fig_h
    ...
    for r, week in enumerate(weeks):
        ...
        for x_hour, y_temp, strike_ct in lightning_markers:
            ax_temp.plot(x_hour, y_temp, marker="*", color=C_LIGHT_EDGE,
                        markersize=8, markeredgecolor=C_LIGHT,
    ...
    footer_bottom_frac = old_bottom_margin_in / fig_h
    ax_foot = fig.add_axes([ax_left, footer_bottom_frac, ax_w,
                            ax_bottom - footer_bottom_frac])
    ax_foot.set_xlim(0, 1)
    ax_foot.set_ylim(0, 1)
    ax_foot.set_axis_off()
    ax_foot.add_patch(mpatches.FancyBboxPatch(
        (0.0, 0.03), 1.0, 0.94, boxstyle="round,pad=0.01",
        facecolor=C_CELL, edgecolor=C_GRID, linewidth=0.7,
        transform=ax_foot.transAxes))
    ax_foot.text(0.015, 0.90, "MONTH SUMMARY", fontsize=9.5,
    ...
    col_xs = [0.02, 0.27, 0.52, 0.77]
    label_w = 0.085
    heading_y = 0.74
    row_ys = [0.56, 0.38, 0.20]
    ...
    # Footer
    fig.text(0.02, 0.005, "Compact monthly weather summary · Daylight band integrated into temp sparkline · Hourly rain & ⚡ lightning overlay",
             ha="left", va="bottom", fontsize=7.5, family="monospace", color=C_TXT)
